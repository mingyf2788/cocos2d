...
bool GameScene2::init()
{
	...
	// 添加调度器
	schedule(schedule_selector(GameScene2::judgeWin), 0.01f, kRepeatForever, 0.1f);

	visibleSize = Director::getInstance()->getVisibleSize();
	origin = Director::getInstance()->getVisibleOrigin();
	lastCid = 'A';

	auto labelRound = Label::createWithTTF("Round 3", "fonts/arial.TTF", 40);
	...
	auto restart = MenuItemImage::create(
		"restart.png",
		"restart.png",
		CC_CALLBACK_1(GameScene2::restartCallback, this));
	...
	// 创建menuItem
	auto item1 = MenuItemLabel::create(label1, CC_CALLBACK_1(GameScene2::moveUp, this));           // W
	auto item2 = MenuItemLabel::create(label2, CC_CALLBACK_1(GameScene2::moveDown, this));           // S
	auto item3 = MenuItemLabel::create(label3, CC_CALLBACK_1(GameScene2::moveLeft, this));           // A
	auto item4 = MenuItemLabel::create(label4, CC_CALLBACK_1(GameScene2::moveRight, this));           // D
...
// 上
void GameScene2::moveUp(Ref* sender) {
...
// 下
void GameScene2::moveDown(Ref* sender) {
...
// 左
void GameScene2::moveLeft(Ref* sender) {
	if (lastCid != 'A') {
...
// 右
void GameScene2::moveRight(Ref* sender) {
	if (lastCid != 'D') {
...
void GameScene2::judgeWin(float dt) {
	if (light1 != nullptr && light2 != nullptr) {
		if (((box1->getBoundingBox().intersectsRect(light1->getBoundingBox())) &&
			...
			// 关闭调度器
			unschedule(schedule_selector(GameScene2::judgeWin));
			// 设置下一关按钮
			auto next = MenuItemImage::create(
				"next.png",
				"next.png",
				CC_CALLBACK_1(GameScene2::nextCallback, this));
			next->setScale(0.1f);
...
//
// 下一关按钮响应函数.
void GameScene2::nextCallback(Ref * pSender) {
	Director::getInstance()->replaceScene(TransitionSlideInR::create(1, GameScene2_1::createScene()));
}
// 重玩按钮响应函数.
void GameScene2::restartCallback(Ref * pSender) {
