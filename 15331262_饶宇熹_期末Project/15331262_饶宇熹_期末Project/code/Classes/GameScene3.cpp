...
void GameScene3::setPhysicsWorld(PhysicsWorld* world) { m_world = world; }
...
bool GameScene3::init()
{
	...
	// 添加调度器
	schedule(schedule_selector(GameScene3::judgeWin), 0.01f, kRepeatForever, 0.1f);

	visibleSize = Director::getInstance()->getVisibleSize();
	origin = Director::getInstance()->getVisibleOrigin();
	lastCid = 'A';

	auto labelRound = Label::createWithTTF("Round 5", "fonts/arial.TTF", 40);
	...
	// 提示语
	auto labeltips = Label::createWithTTF("Where is the ", "fonts/arial.ttf", 40);
	...
	auto restart = MenuItemImage::create(
		"restart.png",
		"restart.png",
		CC_CALLBACK_1(GameScene3::restartCallback, this));
	...
	light2->setPosition(visibleSize.width / 2 + 140, origin.y + 550); //第三左
	addChild(light2, 3);
	...
	// 创建menuItem
	auto item1 = MenuItemLabel::create(label1, CC_CALLBACK_1(GameScene3::moveUp, this));           // W
	auto item2 = MenuItemLabel::create(label2, CC_CALLBACK_1(GameScene3::moveDown, this));           // S
	auto item3 = MenuItemLabel::create(label3, CC_CALLBACK_1(GameScene3::moveLeft, this));           // A
	auto item4 = MenuItemLabel::create(label4, CC_CALLBACK_1(GameScene3::moveRight, this));           // D
...
// 上
void GameScene3::moveUp(Ref* sender) {
...
// 下
void GameScene3::moveDown(Ref* sender) {
...
// 左
void GameScene3::moveLeft(Ref* sender) {
	if (lastCid != 'A') {
...
// 右
void GameScene3::moveRight(Ref* sender) {
	if (lastCid != 'D') {
...
void GameScene3::onKeyPressed(EventKeyboard::KeyCode code, Event* event) {
	switch (code) {
...
void GameScene3::judgeWin(float dt) {
	if (light1 != nullptr && light2 != nullptr) {
		if 	(((box1->getBoundingBox().intersectsRect(light1->getBoundingBox()))&&
			...
			// 关闭调度器
			unschedule(schedule_selector(GameScene3::judgeWin));
			// 设置下一关按钮
			auto next = MenuItemImage::create(
				"next.png",
				"next.png",
				CC_CALLBACK_1(GameScene3::nextCallback, this));
			next->setScale(0.1f);
...
// 下一关按钮响应函数.
void GameScene3::nextCallback(Ref * pSender) {
	Director::getInstance()->replaceScene(GameScene4::createScene());
}
// 重玩按钮响应函数.
void GameScene3::restartCallback(Ref * pSender) {
