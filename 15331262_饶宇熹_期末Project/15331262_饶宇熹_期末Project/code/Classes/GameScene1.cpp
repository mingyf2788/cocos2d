...
void GameScene1::setPhysicsWorld(PhysicsWorld* world) { m_world = world; }
...
bool GameScene1::init()
{
	...
	auto labelRound = Label::createWithTTF("Round 2", "fonts/arial.TTF", 40);
	...
	auto restart = MenuItemImage::create(
		"restart.png",
		"restart.png",
		CC_CALLBACK_1(GameScene1::restartCallback, this));
	restart->setScale(0.1f);
	auto menu2 = Menu::create(restart, NULL);
	menu2->setPosition(750 + origin.x / 2, origin.y + 400);
	...
	bell = Sprite::create("bell.png");
	bell->setPosition(540, 430);
	this->addChild(bell, 1);
	...
	// 创建menuItem
	auto item1 = MenuItemLabel::create(label1, CC_CALLBACK_1(GameScene1::moveUp, this));           // W
	auto item2 = MenuItemLabel::create(label2, CC_CALLBACK_1(GameScene1::moveDown, this));           // S
	auto item3 = MenuItemLabel::create(label3, CC_CALLBACK_1(GameScene1::moveLeft, this));           // A
	auto item4 = MenuItemLabel::create(label4, CC_CALLBACK_1(GameScene1::moveRight, this));           // D
	...
	addCustomListener();
	// 添加调度器
	schedule(schedule_selector(GameScene1::update), 0.05f, kRepeatForever, 0);
	schedule(schedule_selector(GameScene1::judgeWin), 0.01f, kRepeatForever, 0.1f);
	return true;
}

void GameScene1::addPlayer() {
	//使用第一帧创建精灵
...
// 上
void GameScene1::moveUp(Ref* sender) {
...
// 下
void GameScene1::moveDown(Ref* sender) {
...
// 左
void GameScene1::moveLeft(Ref* sender) {
	if (lastCid != 'A') {
...
// 右
void GameScene1::moveRight(Ref* sender) {
	if (lastCid != 'D') {
...
void GameScene1::judgeWin(float dt) {
	if (light1 != nullptr && light2 != nullptr) {
		if (((box1->getBoundingBox().intersectsRect(light1->getBoundingBox())) &&
			...
			// 关闭调度器
			unschedule(schedule_selector(GameScene1::judgeWin));
			// 设置下一关按钮
			auto next = MenuItemImage::create(
				"next.png",
				"next.png",
				CC_CALLBACK_1(GameScene1::nextCallback, this));
			next->setScale(0.1f);
...
void GameScene1::meet(EventCustom* event) {
	int distance = 0;
...
void GameScene1::addCustomListener() {
	auto meetListener = EventListenerCustom::create("meet", CC_CALLBACK_1(GameScene1::meet, this));
	_eventDispatcher->addEventListenerWithFixedPriority(meetListener, 1);
}

void GameScene1::update(float f) {
	EventCustom e("meet");
